...
class CandleSticksTails(ScriptStrategyBase):
    ...
    fake_pair = "ETH-USDT"
    tail_pct_threshold = [0.5, 1, 1.5, 2, 2.5, 3, 4, 5]
    ...
    markets = {"kucoin_paper_trade": {fake_pair}}
    ...
    def get_ticker_kucoin(self):
        ...
        self.log_with_clock(logging.INFO, f"Get Kucoin ticker")
        if records["code"] != "200000":
            self.log_with_clock(logging.INFO, f"Return None")
            return None
        for row in records["data"]["ticker"]:
            if all([row["buy"], row["sell"], row["vol"], row["volValue"]]):
                self.pairs_data[row["symbol"]] = {"bid": Decimal(row["buy"]),
                                                  "ask": Decimal(row["sell"]),
                                                  "vol_base": Decimal(row["vol"]),
                                                  "vol_quote": Decimal(row["volValue"])}
        self.log_with_clock(logging.INFO, f"self.pairs_data_final = {self.pairs_data}")
    ...
    def get_ohlc_bittrex(self):
        ...
        try:
            ...
            self.log_with_clock(logging.INFO, f"Record {url_symbol} data can't be found. Records: {records}")
            return False

    # ---- MEXC ----
    def get_ticker_mexc(self):
        """
        Creates pairs_data dictionary in the following structure:
        'NEO-BTC': {'bid': '0.0005159',
                    'ask': '0.0005169',
                    ...
                    'vol_quote': '1.5022394780543'}
        """
        translation = self.get_symbols_translation_mexc()
    ...
    def get_symbols_translation_mexc(self):
        """
        Returns the dictionary for translating pairs in MEXC format to hummingbot format
        Structure:
            {"BTCUSDT": "BTC-USDT", "ETHBTC": "ETH-BTC"}
        """
        ...
        records = requests.get(url=url).json()
        symbols_translate = {row["symbol"]: f"{row['baseAsset']}-{row['quoteAsset']}" for row in records["symbols"]}
        # Quick fetching all pairs
        # all_symbols = [f"{row['baseAsset']}-{row['quoteAsset']}" for row in records["symbols"]]
        # self.log_with_clock(logging.INFO, f"all_symbols = {all_symbols}")
        # csv_filename = f"all_pairs_mexc.csv"
        # csv_path = os.path.join(data_path(), csv_filename)
        # csv_df = pd.DataFrame(all_symbols)
        # csv_df.to_csv(csv_path, index=False, header=False)
        return symbols_translate

    def get_ohlc_mexc(self):
        ...
        try:
            ...
            self.log_with_clock(logging.INFO, f"Record {self.trading_pair} data can't be found. Records: {records}")
            return False

    # ---- Huobi ----
    def get_ticker_huobi(self):
        """
        Creates pairs_data dictionary in the following structure:
        'NEO-BTC': {'bid': '0.0005159',
                    'ask': '0.0005169',
                    ...
                    'vol_quote': '1.5022394780543'}
        """
        translation = self.get_symbols_translation_huobi()
        url = "https://api.huobi.pro/market/tickers"
        records = requests.get(url=url).json()
        for row in records["data"]:
            if row["symbol"] in translation:
                pair = translation[row["symbol"]]
                self.pairs_data[pair] = {"bid": Decimal(row["bid"]),
                                         "ask": Decimal(row["ask"]),
                                         "vol_base": Decimal(row["amount"]),
                                         "vol_quote": Decimal(row["vol"])}
            else:
                self.log_with_clock(logging.INFO,
                                    f"Symbol {row['symbol']} is not found in the binance translation dict")

    def get_symbols_translation_huobi(self):
        """
        Returns the dictionary for translating pairs in MEXC format to hummingbot format
        Structure:
            {"BTCUSDT": "BTC-USDT", "ETHBTC": "ETH-BTC"}
        ...
        records = requests.get(url=url).json()
        symbols_translate = {row["symbol"]: f"{row['bc'].upper()}-{row['qc'].upper()}" for row in records["data"]}
        return symbols_translate

    def get_ohlc_huobi(self):
        url = f"https://api.huobi.pro/market/history/kline"
        params = {"symbol": (self.trading_pair.replace("-", "")).lower(),
                  "period": "5min",
                  "size": 1000}
        records = requests.get(url=url, params=params).json()
        if records["status"] == "ok":
            try:
                return [{"open": Decimal(str(record["open"])),
                         "high": Decimal(str(record["high"])),
                         "low": Decimal(str(record["low"])),
                         "close": Decimal(str(record["close"]))} for record in records["data"]]
            except Exception as e:
                self.log_with_clock(logging.INFO, f"Record {self.trading_pair} data can't be found. Records: {records}")
                return False
        else:
            self.log_with_clock(logging.INFO, f"Record {self.trading_pair} data status is not ok. Records: {records}")
            return False
        ...
        for pair, data in self.pairs_data.items():
            ...
            if "USD" in quote:
                ...
                if conversion_pair in self.pairs_data:
                    ...
                    self.pairs_data[pair]["volume_usd"] = Decimal("0")
                    continue
    ...
    def on_tick(self):
        ...
        if self.status == "FETCH_TAILS":
            ...
            # self.get_ticker()
            ticker_func_name = f"get_ticker_{self.connector_name}"
            self.log_with_clock(logging.INFO, f"ticker_func_name = {ticker_func_name}")
            ticker_func_method = getattr(self.__class__, ticker_func_name)
            self.log_with_clock(logging.INFO, f"ticker_func_method = {ticker_func_method}")
            ticker_func_method(self)
            self.get_volume_usd()
    ...
    def process_pair(self):
        ...
        get_ohlc_func_name = f"get_ohlc_{self.connector_name}"
        get_ohlc_func_method = getattr(self.__class__, get_ohlc_func_name)
        candles_df = get_ohlc_func_method(self)
        if not candles_df:
            return
        ...
        column_names = ["pair", "volume_usd"] + [f"{pct}" for pct in self.tail_pct_threshold]
